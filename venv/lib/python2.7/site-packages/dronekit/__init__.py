...
class EKF(object):
    ...
    def __init__(self,velocity_variance,terrain_alt_variance,pos_vert_variance,pos_horiz_variance, compass_variance , ekf_ok,flags):
    ...
    def __str__(self):
        fmt = '{} [variance] : velocity ={velocity_variance},terrain_alt={terrain_alt_variance},pos_vert={pos_vert_variance},pos_horiz={pos_horiz_variance},compas={compass_variance},ekf_ok={ekf_ok},flags = {flags}'
        return fmt.format(self.__class__.__name__, **vars(self))
...
class ESC(object):
    """
        ...
        Object has information like rpm, voltage, current and temperatura for all esc in the vehicle
    """



    def __init__(self, esc_s):
        self.velocity_variance = velocity_variance
        self.terrain_alt_variance = terrain_alt_variance
        self.pos_vert_variance = pos_vert_variance
        self.pos_horiz_variance = pos_horiz_variance
    ...
    def __str__(self):
        fmt = '{} [variance] : velocity ={velocity_variance},terrain_alt={terrain_alt_variance},pos_vert={pos_vert_variance},pos_horiz={pos_horiz_variance},compas={compass_variance},ekf_ok={ekf_ok}'
        return fmt.format(self.__class__.__name__, **vars(self))
...
class HasObservers(object):
    ...
    def remove_attribute_listener(self, attr_name, observer):
        ...
        if l is not None:
            print ("aksjdhdf",l,l.__str__(),type(l))
            print ("aaa", type(observer),observer.__str__())
            if observer in l:
...
class Vehicle(HasObservers):
    ...
    def __init__(self, handler):
        ...
        self._default_ready_attrs = ['parameters', 'gps_0', 'armed', 'mode', 'attitude','ekf']
        ...
        @self.on_message('ESC_TELEMETRY_1_TO_4')
        def listener(self,name,m):
            print ("Motooooores " , dir(o))
            self.notify_attribute_listeners('ESC', self.ESC)

        @self.on_message('MAV_SYS_STATUS_SENSOR ')
        def listener(self,name,m):
            print ("FENCES " , dir(o))
            # self.notify_attribute_listeners('ESC', self.ESC)
        ...
        self._velocity_variance = 0
        self._terrain_alt_variance = 0
        self._pos_vert_variance = 0
        self._pos_horiz_variance = 0
        self._compass_variance = 0
        self._flags = 0
        ...
        def listener(self, name, m):
            ...
            # values: EKF is based on this values to return its result
            self._velocity_variance = m.velocity_variance
            self._terrain_alt_variance = m.terrain_alt_variance
            self._pos_vert_variance = m.pos_vert_variance
            self._pos_horiz_variance = m.pos_horiz_variance
            self._compass_variance = m.compass_variance
            self._flags = m.flags

            self.notify_attribute_listeners('ekf', self.ekf, cache=True)
    ...
    @property
    def ekf(self):
        return EKF( self._velocity_variance, self._terrain_alt_variance, self._pos_vert_variance, self._pos_horiz_variance,
                    self._compass_variance, self.ekf_ok,self._flags)
    ...
    def armed(self, value):
        print ("Estoy en la libreria de DroneKit" , value, self._armed , self.is_armable)
